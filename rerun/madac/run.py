...
from madac.utils.logging import Logger
from madac.utils.timehelper import time_left, time_str
from os.path import dirname, abspath

from madac.learners import REGISTRY as le_REGISTRY
from madac.runners import REGISTRY as r_REGISTRY
...
def run(_run, _config, _log):
    ...
    try:
        unique_token = f"{_config['name']}_seed{_config['seed']}_rnn{str(_config['use_rnn'])}_" \
                       f"ban{str(_config['env_args']['ban_agent'])}_R{str(_config['env_args']['reward_type'])}_" \
                       f"{map_name}_{datetime.datetime.now()}"
    except:
        unique_token = f"{_config['name']}_seed{_config['seed']}_rnn{str(_config['use_rnn'])}_" \
                       f"{map_name}_{datetime.datetime.now()}"
    ...
    while runner.t_env <= args.t_max:
        ...
        if args.save_model and (
            ...
            save_path = os.path.join(
                args.local_results_path, "madac", "models", args.unique_token, str(runner.t_env))
