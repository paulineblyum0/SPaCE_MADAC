...
def env_worker(remote, env_fn, seed=None):
    # MamoBase ignores its `seed` kwarg and draws from the global RNGs, so the
    # per-worker stream has to be fixed here, before the env is constructed.
    if seed is not None:
        random.seed(seed)
