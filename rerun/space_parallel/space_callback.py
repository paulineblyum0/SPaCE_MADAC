...
class UpdateEnvCallbackParallel(BaseCallback):
    def __init__(
        ...
        # None (not 0.0) so the first boundary records a baseline instead of
        # being scored against a zero threshold, which can only ever return
        # "unstable" and wastes one of the ~11 usable checks.
        self.last_q = None

        # True until the first policy update has run. Read at
        # _on_rollout_start, where "a rollout has ended" and "the policy has
        # been updated" coincide -- see module docstring point 2.
    ...
    def _on_training_start(self) -> None:
        if self.use_space == space_operation.NO_SPACE:
            return
        # Initial curriculum: size 1, random ordering (no policy to probe yet).
    ...
    def _on_rollout_start(self) -> None:
        ...
        # Single probe pass per boundary, shared by the size decision and the
        # ordering decision.
    ...
    def _on_rollout_end(self) -> None:
        self.space_logger.info(
            "Rollout collected at %d timesteps (policy update follows)",
            self.num_timesteps,
        )
    ...
    def _update_curriculum_size(self, evals) -> None:
        ...
        # collected, not over instances 0..size-1.
        mean_q = float(np.mean([evals[i] for i in self.active_curriculum]))

        if self.last_q is None:
            self.last_q = mean_q
            self.space_logger.info(
                "First size check skipped; baseline mean_q=%.6f", mean_q
            )
            return
    ...
    def _order_instances_qvals(self, evals):
        # Ascending, matching baselines_spl.py order_instances_qvals.
    ...
    # ------------------------------------------------------------------
    # Value-function queries -- always against the probe env
    # ------------------------------------------------------------------
