"""
Each RQ defines what it's comparing. This file turns it into the corresponding tables and plots. 
"""
...
def _panel(series, problems, loader, out_path, xlabel, compact_x=False, colours=None, band=BAND):
    ...
    def draw(ax, problem):
        for i, label in enumerate(series):
            ...
            if x is None:
                continue
            figstyle.plot_band(ax, x, Y, label, index=i, band=band, colours=colours)

    figstyle.panel_figure(problems, draw, xlabel=xlabel, ylabel="IGD", out_path=out_path, compact_x=compact_x)


def _avg_panel(series, problems, out_path, xlabel, compact_x=False, colours=None, metric=None):
    # one line per algorithm averaged across `problems`
    colours = colours or figstyle.palette_for(series)

    def draw(ax, _):
        for i, label in enumerate(series):
            x, Y = data.training_curve_matrix_avg(series, label, problems, metric=metric or data.METRIC)
            if x is None:
                continue
            figstyle.plot_band(ax, x, Y, label, index=i, band=BAND,colours=colours)

    figstyle.panel_figure(["avg"], draw, xlabel=xlabel, ylabel="IGD", out_path=out_path, compact_x=compact_x)
...
def run(name, series, eval_problems, train_problems=None,
    ...
    if within_episode:
        ...
        _panel(series, eval_problems, loader, OUT / f"{name}_within_episode.pdf", "Generation", colours=colours)

    if training_curve:
        loader = partial(data.training_curve_matrix, metric=metric or data.METRIC)
        _panel(series, train_problems, loader, OUT / f"{name}_training_curve.pdf",
            "Timesteps", compact_x=True, colours=colours, band=training_curve_band)

    if training_curve_avg:
        _avg_panel(series, train_problems,OUT / f"{name}_training_curve_avg.pdf",
