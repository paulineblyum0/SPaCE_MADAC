"""
Every number that goes in an evaluration table.

  final_igd_tables(): pooled + per-seed descriptives, no test
  rq2_statistics(): pooled rank-sum per pair, secondary check
  rq1_final_igd_tables(): paired per-seed signed-rank test
  rq3_statistics(): paired per-seed signed-rank, specific pairs
...
def final_igd_tables(series, problems, prefix="", metric=None):
    # IGD table of final policy evaluation
...
def rq2_statistics(series, problems, prefix="rq2_"):
    #pooled rank-sum per pair per problem, Holm-corrected within each problem.
    rows = []
...
def rq1_final_igd_tables(series, problems, prefix="rq1_"):
    # Paired per-seed signed-rank test, Holm-corrected across the 3 seeds.
    labels = list(series)
    if len(labels) != 2:
        raise ValueError(f"rq1_final_igd_tables expects exactly 2 conditions, got {labels}")
    a, b = labels  # a = "adaptive_open=False", b = "adaptive_open=True"
    ...
    for problem in problems:
        ...
        # paired per seed, one-sided (True < False), Holm across seeds
        pvals, directions = [], []
    ...
    # column order follows seeds_a's order == data.SEEDS
...
def rq3_statistics(series, problems, pairs, prefix="rq3_"):
    # Paired per-seed Wilcoxon signed-rank per seed per problem (not pooled across seeds), Holm within each.
    rows = []
    ...
    for problem in problems:
        ...
        for label in series:
            X, seeds = data.final_igd_matrix(series, label, problem)
            if X is not None:
                loaded[label] = dict(zip(seeds, X))  # each seed has 30 repeats, index-aligned
    ...
    OUT.mkdir(parents=True, exist_ok=True)
    write_csv(OUT / f"{prefix}statistics.csv",
              ["problem", "algorithm_a", "algorithm_b", "seed","mean_a", "mean_b", "lower_mean",
               "p_adj", "sig", "significant_seeds"], rows)



def auc_table(series, problems, prefix=""):
    # Time-averaged IGD over training, one value per seed
    rows = []
    for problem in problems:
        curves = {}
        for label in series:
