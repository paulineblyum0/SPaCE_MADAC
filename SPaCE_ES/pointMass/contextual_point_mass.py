...
class ContextualPointMass(Env):
    def __init__(self, context=np.array([0.0, 2.0, 2.0])):
        ...
        self._state = None
        self._goal_state = np.array([0.0, 0.0, -3.0, 0.0])
        self.context = context
        self._dt = 0.01
        # self._viewer = Viewer(8, 8, background=(255, 255, 255))

    def reset(self):
        self._state = np.array([0.0, 0.0, 3.0, 0.0])
    ...
    def step(self, action):
        ...
        info = {
            "success": np.linalg.norm(self._goal_state[0::2] - new_state[0::2])
            < 0.25
        }
        ...
        return (
            new_state,
            np.exp(
                -0.6 * np.linalg.norm(self._goal_state[0::2] - new_state[0::2])
            ),
            crash,
            False,
            info,
    ...
    def render(self, mode="human"):
        ...
        self._viewer.circle(
            self._state[0::2] + np.array([4.0, 4.0]), 0.1, color=(0, 0, 0)
        )
